...
function CategoryAccordion({
  ...
  return (
    <div>
      ...
      {isOpen && (
        <div
          className={`px-6 pb-4 ${isDark ? "border-t border-gray-700/50" : "border-t border-gray-100"}`}
        >
          {category.chummeSubCategories &&
          category.chummeSubCategories.length > 0 ? (
            ...
            <p
              className={`mt-3 text-sm ${isDark ? "text-gray-500" : "text-gray-400"}`}
            >
              No subcategories
            </p>
          )}

          {category.discoveryKeywords &&
            category.discoveryKeywords.length > 0 && (
              <div
...
function SubCategoryAccordion({
  ...
  return (
    <div
      className={`rounded-lg border ${isDark ? "border-gray-700 bg-gray-900/50" : "border-gray-200 bg-gray-50"}`}
    >
      <button
        onClick={() => setOpen(!open)}
      ...
      {isOpen && (
        <div
          className={`px-4 pb-3 ${isDark ? "border-t border-gray-700/50" : "border-t border-gray-200"}`}
        >
          {topics.length > 0 ? (
